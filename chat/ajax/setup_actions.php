<?php
	...
	else if ( $action == "update_profile" )
	{
		...
		else
		{
			...
			else if ( $md5_password == md5( "$npassword$vpassword".$_COOKIE["phplive_token"] ) )
			{
				$password_query = "" ;
	...
	else if ( $action == "view_invite" )
	{
		...
		if ( touch( "$CONF[TYPE_IO_DIR]/$vis_token.txt" ) )
			$json_data = "json_data = { \"status\": 1 };" ;
		else
			$json_data = "json_data = { \"status\": 0 };" ;
	}
	else if ( $action == "fetch_message" )
	{
		...
		if ( isset( $message["messageID"] ) )
		{
			...
			$json_data = "json_data = { \"status\": 1, \"subject\": \"$subject\", \"name\": \"$message[vname]\", \"email\": \"$message[vemail]\", \"to\": \"$to\", \"subject\": \"$subject\", \"created\": \"$created\", \"custom\": \"$message[custom]\", \"message\": \"$body\" };" ;
		}
		else
