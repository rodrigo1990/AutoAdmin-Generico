...
<!--
	...
	function open_message( themessageid, thestatus )
	{
		...
		data: "action=fetch_message&ses=<?php echo $ses ?>&messageid="+themessageid+"&unique="+unique,
		success: function(data){
			eval( data ) ;

			if ( json_data.status )
			{
				var custom_raw = json_data.custom ;
				custom_raw = custom_raw.split("-cus-") ;
				var custom_string = "<table cellspacing=0 cellpadding=0 border=0 width=\"100%\"><tr><td nowrap colspan=2><div class=\"info_neutral\"><img src=\"../pics/icons/pin_note.png\" width=\"14\" height=\"14\" border=\"0\" alt=\"\"> Custom Variables</div></td></tr>" ;
				...
				custom_string += "</table>" ;
				if ( custom_raw.length > 1 ) { $('#custom_variables').html( custom_string ) }
				else { $('#custom_variables').html("") }
		...
		error:function (xhr, ajaxOptions, thrownError){
			do_alert( 0, "Error retrieving message.  Please reload the page and try again." ) ;
		} });
	}
	...
	function delete_message()
	{
		...
		if ( global_messageid && confirm( "Really delete this message?" ) )
		{
			...
			type: "POST",
			url: "../ajax/setup_actions.php",
			data: "action=delete_message&ses=<?php echo $ses ?>&messageid="+global_messageid+"&unique="+unique,
			success: function(data){
				eval( data ) ;
	...
	function lock_message( theobject, themessageid )
	{
		...
		type: "POST",
		url: "../ajax/setup_actions.php",
		data: "action=lock_message&ses=<?php echo $ses ?>&messageid="+themessageid+"&lock="+lock+"&unique="+unique,
		success: function(data){
			eval( data ) ;
		...
		error:function (xhr, ajaxOptions, thrownError){
			do_alert( 0, "Error locking message.  Please reload the page and try again." ) ;
		} });
	}
	...
	function do_savem( thevalue )
	{
		if ( confirm( "Update automatic delete setting?" ) )
		{
			...
			$('#savem_'+global_savem).prop('checked', true) ;
	}
			...
			<div style="width: 700px; margin: 0 auto; margin-top: 50px;" class="info_info">
				<div id="div_message_body" class="round">
					<div>
						...
						<tr>
							<td width="100" align="right"><div style="background: #EAEAEA; border: 1px dashed #A3A3A3;" class="td_dept_td_blank round">Subject</div></td>
							<td width="100%"><div class="td_dept_td_blank" id="msg_subject" style="font-weight: bold; font-size: 14px;"></div></td>
							<td rowspan="3" valign="top"><div class="info_info" style="font-size: 10px;" id="custom_variables"></div></td>
						</tr>
						<tr>
							<td width="100" align="right"><div style="background: #EAEAEA; border: 1px dashed #A3A3A3;" class="td_dept_td_blank round">From</div></td>
							<td><div class="td_dept_td_blank"><span id="msg_from_name"></span> &lt;<span id="msg_from_email"></span>&gt;</div></td>
						</tr>
						<tr>
							<td width="100" align="right"><div style="background: #EAEAEA; border: 1px dashed #A3A3A3;" class="td_dept_td_blank round">To</div></td>
							<td><div class="td_dept_td_blank" id="msg_to"></div></td>
						</tr>
						<tr>
							<td width="100" align="right" valign="top">
								<div style="background: #EAEAEA; border: 1px dashed #A3A3A3;" class="td_dept_td_blank round">Message</div>
							</td>
							<td valign="top">
						...
						<tr>
							<td nowrap><button type="button" onClick="close_message()">close window</button></td>
							<td colspan=2 align="right"><div onClick="delete_message()" style="margin-top: 15px; cursor: pointer;"><img src="../pics/btn_delete.png" width="64" height="23" border="0" alt=""></div></td>
						</tr>
						</table>
					</div>
				</div>
