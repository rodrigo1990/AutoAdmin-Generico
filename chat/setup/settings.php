...
<script type="text/javascript" src="../js/jquery_cookie.js?<?php echo $VERSION ?>"></script>
...
<!--
	...
	function show_div( thediv )
	{
		...
		var divs = Array( "logo", "themes", "embed", "charset", "time", "eips", "sips", "cookie", "screen", "profile" ) ;
	...
	function update_profile()
	{
		...
		{
			if ( $('#'+inputs[c]).val() == "" ){ $('#status_'+inputs[c]).empty().html( "&nbsp; <img src=\"../pics/icons/alert.png\" width=\"12\" height=\"12\" border=\"0\" alt=\"Please provide a value.\" title=\"Please provide a value.\">" ) ; execute = 0 ; }
		}
	...
	function update_profile_doit()
	{
		...
		var md5_password = phplive_md5( npassword+vpassword+$.cookie("phplive_token") ) ;
	...
	function confirm_change( theflag )
	{
		if ( global_cookie != theflag )
		{
			if ( confirm( "Update visitor cookies to "+theflag+"?" ) )
			{
				...
				$('#cookie_'+global_cookie).prop('checked', true) ;
		}
	}
	...
	function confirm_charset( thecharset )
	{
		if ( global_charset != thecharset )
		{
			if ( confirm( "Update operator console charset to "+thecharset+"?" ) )
			{
				...
				$('#charset_'+global_charset).prop('checked', true) ;
		}
	}
	...
	function confirm_popout( thepopout )
	{
		...
		{
			if ( confirm( "Switch "+thepopout+" embed chat popout?" ) )
			{
				$.ajax({
					type: "POST",
					url: "../ajax/setup_actions.php",
				...
				$('#popout_'+global_popout).prop('checked', true) ;
		}
	}
	...
	function check_logo_dim()
	{
		...
		img.src = '<?php print Util_Upload_GetLogo( "$CONF[UPLOAD_HTTP]", $deptid ) ?>' ;
	}
...
<?php include_once( "./inc_header.php" ) ?>
		...
		<div class="op_submenu_wrapper">
			...
			<div class="op_submenu" onClick="show_div('embed')" id="menu_embed">Chat</div>
			...
			<div class="op_submenu" onClick="show_div('sips')" id="menu_sips">Block IPs</div>
		...
		<div style="display: none; margin-top: 25px;" id="settings_logo">
			...
			<div style="margin-top: 15px;" class="info_neutral"><img src="../pics/icons/info.png" width="12" height="12" border="0" alt=""> To maximize visible chat space, the logo will not be displayed with the <a href="icons.php?ses=<?php echo $ses ?>&jump=settings">embed chat setting</a>.</div>
			...
			<tr>
				<td valign="top">
					...
					<?php else: ?>
						...
						<div id="div_logo" style="border: 1px solid #DFDFDF; margin-top: 25px; background: url( <?php print Util_Upload_GetLogo( $CONF["UPLOAD_HTTP"], $deptid ) ?> ) no-repeat;">&nbsp;</div>
		...
		<div style="display: none; margin-top: 25px; text-align: justify;" id="settings_themes">
			<div>Visitor chat window theme preference.  Operators will be able to set their own themes by logging into the operator area.</div>
			<div style="margin-top: 25px;"><img src="../pics/icons/arrow_right.png" width="16" height="15" border="0" alt=""> <a href="depts.php?ses=<?php echo $ses ?>">Visit the departments area to set the theme for each department.</a></div>
		</div>

		<div style="display: none; margin-top: 25px;" id="settings_embed">
			If <a href="icons.php?ses=<?php echo $ses ?>&jump=settings">embed chat</a> is enabled, switch on/off the embed chat "popout" feature.  The "popout" feature enables the visitor to open the embed chat in a new window when clicking the "popout" icon <img src="../themes/initiate/win_pop.png" width="16" height="16" border="0" alt="">.  By switching off the embed chat "popout", the chat "popout" icon <img src="../themes/initiate/win_pop.png" width="16" height="16" border="0" alt=""> will not be visible.

			<div style="margin-top: 25px;">
				<div class="li_op round"><input type="radio" name="popout" id="popout_on" value="on" onClick="confirm_popout(this.value)" <?php echo ( $VALS["POPOUT"] != "off" ) ? "checked" : "" ?>> Embed Chat Popout On</div>
				<div class="li_op round"><input type="radio" name="popout" id="popout_off" value="off" onClick="confirm_popout(this.value)" <?php echo ( $VALS["POPOUT"] == "off" ) ? "checked" : "" ?>> Embed Chat Popout Off</div>
				<div style="clear: both;"></div>
			</div>
		</div>
			...
			<div style="margin-top: 25px;">
				...
				<tr>
					<td valign="top" nowrap style="padding-right: 25px;">
						<div class="info_box">
							<div>Your IP: <span class="txt_orange"><?php echo $your_ip ?></span></div>
