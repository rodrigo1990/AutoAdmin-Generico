<?php
	...
	else if ( $action == "clear_api" )
	{
		...
		HEADER( "location: ./extras_geo.php?ses=$ses&jump=$jump&action=cleared_api" ) ;
		exit ;
	}
...
<!--
	...
	function check_geo_files( theindex, thepercent )
	{
		...
		success: function(data){
			...
			if ( !json_data.status && ( typeof( json_data.error ) == "undefined" ) )
			{
				...
				st_geo_import = setTimeout( function(){check_geo_files( theindex+1, json_data.percent ) ; }, 400 ) ;
			}
	...
	function lookup_ip( theip )
	{
		...
		else
		{
			...
			type: "POST",
			url: "./extras_geo.php",
			data: "action=lookup&ses=<?php echo $ses ?>&ip="+theip+"&unique="+unique,
			success: function(data){
				eval( data ) ;
			...
			error:function (xhr, ajaxOptions, thrownError){
				do_alert( 0, "Error processing IP.  Please reload the page and try again." ) ;
			} });
		}
	}
