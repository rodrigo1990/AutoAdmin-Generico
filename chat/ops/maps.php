<?php
	...
	$skip = Util_Format_Sanatize( Util_Format_GetVar( "skip" ), "ln" ) ;
	...
	else
	{
		...
		$geo_country = ( $geo_country_code != "unknown" ) ? $countries[$geo_country_code] : "Location Unknown" ;
...
<body id="chat_info_body" style="margin: 0px; border: 0px; padding: 0px;">
	...
	<div id="map_default" style="display: none; height: 100%;">
		<?php if ( $skip ): ?>
		<div class="info_box">Location not available for this session.</div>
		...
		<?php else: ?>
			...
			<?php else: ?>
			<div class="chat_info_td">To enable GeoIP Location, contact the setup admin.  The admin will need to login to the setup area and access the top menu "Extras".  On the "Extras" page, click the "GeoIP" sub menu.</td>
			<?php endif ; ?>
