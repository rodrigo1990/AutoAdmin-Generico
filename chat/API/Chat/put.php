<?php
					...
					$sim_ops = "" )
	{
		...
		LIST( $deptid, $opid, $status, $initiate, $op2op, $t_vses, $os, $browser, $ces, $resolution, $vname, $vemail, $ip, $vis_token, $vis_token_, $onpage, $title, $question, $marketid, $refer, $custom, $auto_pop, $sim_ops ) = database_mysql_quote( $deptid, $opid, $status, $initiate, $op2op, $t_vses, $os, $browser, $ces, $resolution, $vname, $vemail, $ip, $vis_token, $vis_token_, $onpage, $title, $question, $marketid, $refer, $custom, $auto_pop, $sim_ops ) ;
		...
		else
		{
			...
			$requests = Chat_get_IPTotalRequests( $dbh, $vis_token ) ;
			if ( !$requests ) { $requests = 1 ; }

			$query = "INSERT INTO p_requests VALUES ( NULL, $now, 0, $now, $vupdated, $status, $auto_pop, $initiate, $deptid, $opid, $op2op, $t_vses, $marketid, $os, $browser, $requests, '$ces', '$resolution', '$vname', '$vemail', '$ip', '$vis_token', '$vis_token_', '$sim_ops', '', '$onpage', '$title', '$rstring', '$refer', '$custom', '$question' )" ;
					...
					$requestid )
	{
		...
		$query = "INSERT INTO p_req_log ( ces, created, ended, status, initiated, deptID, opID, op2op, marketID, os, browser, resolution, vname, vemail, ip, md5_vis, sim_ops, onpage, title, custom, question ) SELECT ces, created, 0, status, initiated, deptID, opID, op2op, marketID, os, browser, resolution, vname, vemail, ip, md5_vis_, sim_ops, onpage, title, custom, question FROM p_requests WHERE p_requests.requestID = $requestid" ;
		database_mysql_query( $dbh, $query ) ;
