<?php
					...
					$stat_end )
	{
		...
		$query = "SELECT count(*) AS total, onpage FROM p_footprints WHERE created >= $stat_start AND created <= $stat_end GROUP BY md5_page ORDER BY total DESC LIMIT 25" ;
		database_mysql_query( $dbh, $query ) ;
					...
					$stat_end )
	{
		...
		$query = "SELECT SUM(total) AS total, refer FROM p_referstats WHERE sdate >= $stat_start AND sdate <= $stat_end GROUP BY md5_page ORDER BY total DESC LIMIT 100" ;
		database_mysql_query( $dbh, $query ) ;
					...
					$vis_token )
	{
		...
		LIST( $vis_token ) = database_mysql_quote( $vis_token ) ;
