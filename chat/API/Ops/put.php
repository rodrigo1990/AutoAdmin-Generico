<?php
					...
					$email )
	{
		...
		LIST( $opid, $status, $rate, $sms, $op2op, $traffic, $viewip, $maxc, $password, $name, $email ) = database_mysql_quote( $opid, $status, $rate, $sms, $op2op, $traffic, $viewip, $maxc, $password, $name, $email ) ;
		...
		if ( isset( $operator["opID"] ) )
			...
			$query = "REPLACE INTO p_operators VALUES ( $opid, 0, 0, 0, 0, $rate, $op2op, $traffic, $viewip, 1, 0, $maxc, '', '', 0, $sms, '', '$login', '$password', '$name', '$email', '', 'default', 'default', 'default' )" ;
