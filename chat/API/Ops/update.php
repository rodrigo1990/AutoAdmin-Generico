<?php
	...
	FUNCTION Ops_update_CurcValues( &$dbh )
	{
		$query = "UPDATE p_operators SET curc = 0" ;
		database_mysql_query( $dbh, $query ) ;
		...
		for ( $c = 0; $c < count( $ops ); ++$c )
		{
			$op = $ops[$c] ;
			$query = "UPDATE p_operators SET curc = $op[total] WHERE opID = $op[opID]" ;
			database_mysql_query( $dbh, $query ) ;
		}
