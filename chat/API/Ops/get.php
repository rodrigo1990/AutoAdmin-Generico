<?php
					...
					$rstring )
	{
		...
		$query = "SELECT p_operators.opID AS opID, p_operators.rate AS rate, p_operators.sms AS sms, p_operators.smsnum AS smsnum, p_operators.name AS name, p_operators.email AS email FROM p_operators INNER JOIN p_dept_ops ON p_operators.opID = p_dept_ops.opID WHERE p_operators.status = 1 AND p_operators.lastactive > $lastactive AND p_operators.curc != p_operators.maxc AND p_dept_ops.deptID = $deptid $rstring_query $order_by LIMIT 1" ;
		database_mysql_query( $dbh, $query ) ;
