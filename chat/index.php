<?php
	...
	if ( $action == "submit" )
	{
		...
		else
		{
			...
			if ( isset( $opinfo["opID"] ) && ( $password == md5($opinfo["password"].$token) ) )
			{
				...
				if ( $remember ) { setcookie( "phplive_adminLogin", $opinfo['login'], time()+(60*60*24*1095) ) ; }
				else { setcookie( "phplive_adminLogin", "", time()-1 ) ; }
	...
	else if ( $action == "logout" )
	{
		...
		if ( $menu == "sa" )
		{
			...
			if ( isset( $_COOKIE["phplive_adminID"] ) ) { Ops_update_ext_AdminValue( $dbh, Util_Format_Sanatize( $_COOKIE["phplive_adminID"], "n" ), "ses", "" ) ; setcookie( "phplive_adminID", FALSE ) ; }
		}
		else
		{
			include_once( "$CONF[DOCUMENT_ROOT]/API/Ops/update.php" ) ;

			if ( isset( $_COOKIE["phplive_opID"] ) ) { Ops_update_putOpStatus( $dbh, Util_Format_Sanatize( $_COOKIE["phplive_opID"], "n" ), 0 ) ; Ops_update_OpValue( $dbh, Util_Format_Sanatize( $_COOKIE["phplive_opID"], "n" ), "status", 0 ) ; Ops_update_OpValue( $dbh, Util_Format_Sanatize( $_COOKIE["phplive_opID"], "n" ), "ses", "" ) ; setcookie( "phplive_opID", FALSE ) ; }
		}

		// main one included at chat_actions_op_status.php - put it here as well as placing
		// at action=logout does not work for too fast of query.
	...
	if ( !$login && isset( $_COOKIE["phplive_adminLogin"] ) && $_COOKIE["phplive_adminLogin"] )
		$login = Util_Format_Sanatize( $_COOKIE["phplive_adminLogin"], "ln" ) ;
?>
...
<!--
	...
	var global_menu ;
	<?php if ( $action == "auto_login" ): ?>location.href = "./?action=submit&phplive_login=<?php echo $login ?>&phplive_password="+phplive_md5( phplive_md5('<?php echo $password ?>')+$.cookie("phplive_token") )+"&menu=<?php echo $menu ?>&auto=<?php echo $auto ?>" ;<?php endif ; ?>
	...
	function do_login()
	{
		...
		{
			var md5_password = phplive_md5( phplive_md5($('#phplive_password').val())+$.cookie("phplive_token") ) ;
...
<div id="body" style="padding-bottom: 60px;">
	<div style="width: 100%; padding-top: 50px;">
		...
		<div class="info_info" style="width: 480px; height: 250px; margin: 0 auto; padding: 10px; text-shadow: 1px 1px #FFFFFF;">
			...
			<tr>
				<td></td>
				<td colspan=3><div id="login_remember" style="display: none; padding-top: 15px;"><input type="checkbox" name="remember" id="remember" value=1 <?php echo ( isset( $_COOKIE["phplive_adminLogin"] ) && $_COOKIE["phplive_adminLogin"] ) ? "checked" :  "" ; ?>> remember login</div></td>
			</tr>
			<tr>
				<td></td>
				<td colspan=3>
		...
		<div style="background: url( ./pics/bg_fade_lite.png ) no-repeat; background-position: top center; width: 500px; height: 19px; margin: 0 auto; opacity:0.3; filter: alpha(opacity=30)" class="round_top">&nbsp;</div>
