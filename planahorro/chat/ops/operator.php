	...
	for ( $c = 0; $c < count( $dir_files ); ++$c )
	{
		...
		if ( $modtime < ( $now - (60*60*14) ) )
		{
			if ( is_file( $file ) )
			{
				...
				if ( isset( $matches[1] ) )
				{
					$ces = Util_Format_Sanatize( substr( $matches[1], -32 ), "ln" ) ;
					...
					if ( isset( $requestinfo["ces"] ) && !$requestinfo["ended"] )
					{
						...
						if ( isset( $output[0] ) )
						{
							...
							{
								Chat_put_itr_Transcript( $dbh, $ces, $requestinfo["status"], $requestinfo["created"], $modtime, $requestinfo["deptID"], $requestinfo["opID"], $requestinfo["initiated"], $requestinfo["op2op"], 0, $fsize, $requestinfo["vname"], $requestinfo["vemail"], $requestinfo["ip"], $requestinfo["md5_vis"], $requestinfo["question"], $formatted, $plain ) ;
							}
						}
					}
				}
...
<!--
	...
	var st_resize, st_typing, st_reconnect, st_rating, st_flash_console ;
	...
	var console_blink_r = <?php echo $console_blink ?> ;
	...
	function prep_init()
	{
		<?php if ( !preg_match( "/NT 6.1/i", $agent ) ): ?>
		$("#chat_input").dblclick(function() {
			toggle_input_text() ;
		});
		<?php endif ; ?>
	}
			...
			success: function(data){
				...
				if ( json_data.status )
				{
					...
					else
					{
						...
						chats[ces]["status"] = 1 ;
						var transcript = chats[ces]["trans"] ;
						if ( ( typeof( cans[auto_can_title] ) != "undefined" ) && ( chats[ces]["status"] != 2 ) && !chats[ces]["op2op"] )
						{
							select_canned_pre( auto_can_title ) ;
							add_text_prepare() ;
						}
		...
		success: function(data){
			...
			if ( json_data.status )
			{
				...
				for ( c = 0; c < json_data.cans.length; ++c )
				{
					...
					if ( parseInt( json_data.cans[c]["auto_select"] ) ) { auto_can_title = json_data.cans[c]["title"] ; }
		...
		if ( ( typeof( ces ) != "undefined" ) && ( typeof( chats[ces] ) != "undefined" ) )
		{
			...
			else if ( thediv == "transfer" )
			{
				if ( ( chats[ces]["status"] == 1 ) && !chats[ces]["op2op"] && !chats[ces]["disconnected"] )
					...
					$('#info_transfer').empty().html( "<div class=\"info_box\">A chat session must be active.</div>" ) ;
				else
				...
				success: function(data){
					...
					if ( json_data.status )
					{
						...
						if ( json_data.transcripts.length == 0 )
							transcripts_string = "<div class=\"chat_info_td\">Blank results.</div>" ;
	...
	{
		var iframe_src = document.getElementById('iframe_extras').contentWindow.location + "" ; 
		if ( iframe_src.indexOf( "op_extras" ) != -1 ) { init_iframe( 'iframe_extras' ) ; }
		else
		{
			$('#iframe_extras').attr('src', "op_extras.php?ses=<?php echo $ses ?>&"+unixtime() ).ready(function() {
				init_iframe( 'iframe_extras' ) ;
			});
		}
		$('#chat_extra_body_extras').fadeIn("fast") ;
	}
	...
	function toggle_status( thestatus )
	{
		...
		if ( prev_status != thestatus )
		{
			...
			{
				if ( typeof( si_offline ) != "undefined" ) { clearInterval( si_offline ); si_offline = this.undefined ; $('#offline_timer').html( logout_timer+":00" ) ; }
				if ( thestatus == 0 )
				{
				...
				else
				{
					...
					if ( prev_status == 0 ) { prev_status = this.undefined ; toggle_status( 0 ) ; }
					else if ( prev_status ) { prev_status = this.undefined ; toggle_status( 1 ) ; }
				}
			}
		}
	}
	...
	function update_ratings_doit()
	{
		...
		success: function(data){
			...
			if ( json_data.status )
			{
				$('#rating_recent').html( stars[json_data.rating_recent] ).unbind('click').bind('click', function() {
					if ( json_data.ces )
						open_transcript( json_data.ces ) ;
				});
				$('#rating_overall').html( stars[json_data.rating_overall] ) ;

				if ( fetch_rating_flag )
				{
					$('#chats_today').html( json_data.chats_today+" accepted" ) ;
					$('#chats_overall').html( json_data.chats_overall+" accepted" ) ;
				}
			}
			...
			else if ( parseInt( json_data.signal ) == 2 )
			{
				...
				si_automatic_offline = setInterval(function(){
					if ( !total_chats() )
					{
						clearInterval( si_automatic_offline ) ; si_automatic_offline = this.undefined ;
	...
	{
		++reconnect_counter ;
		if ( reconnect_counter == 1 ) { $('#idle_timer_notice').hide() ; for ( thisces in chats ) { idle_reset( thisces ) ; } }
...
<div style="position: absolute; top: 20px; padding: 10px; z-Index: 2;" onClick="input_focus(); close_extra( extra );">
	...
	<div id="chat_input" style="margin-top: 8px;" onClick="clear_flash_console()">
		<textarea id="input_text" rows="3" style="padding: 2px; height: 75px; resize: none;" wrap="virtual" onFocus="dn_close();" onKeyup="input_text_listen(event);" onKeydown="input_text_typing(event);" disabled></textarea>
	</div>
</div>
...
<div id="chat_footer" style="position: absolute; width: 100%; bottom: 0px; height: 25px; z-Index: 102;">
	...
	<div id="chat_footer_cell_extras" class="chat_footer_cell" onClick="toggle_extra( 'extras', '', '', 'Extras' )">Extras</div> -->
