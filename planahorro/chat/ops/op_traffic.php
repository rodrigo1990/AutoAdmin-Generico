...
<!--
	...
	function populate_footprint( thevis_token, theduration, themarket, theip, theos, thebrowser, theresolution, thet_footprints, thet_requests, thet_initiates, thetitle, theonpage, therefer_snap, therefer_raw, thecountry, theregion, thecity )
	{
		...
		var height_document = $('#footprint_info_wrapper').outerHeight() - 250 ;
		//if ( height_document > 180 ) { height_document = 250 ; }
		$('#info_footprints').css({'height': height_document}) ;
	...
	function populate_requestinfo( theip, thevis_token, thecountry, theregion, thecity )
	{
		...
		success: function(data){
			...
			if ( json_data.status )
			{
				$('#info_duration').empty().html( " <span class=\"info_good\"><img src=\"../themes/<?php echo $opinfo["theme"] ?>/info_chats.gif\" width=\"10\" height=\"10\" border=\"0\" alt=\"\" style=\"cursor: help;\" title=\"currently in a chat session with "+json_data.name+"\" alt=\"currently in a chat session with "+json_data.name+"\"></span>" ) ;
			}
				...
				<div style="margin-top: 15px;">
					...
					<tr>
						<td valign="top">
							...
							<div style="margin-top: 5px; font-size: 10px;">* brief message (recommended: 25 or fewer words)</div>
						</td>
						<td valign="top">
