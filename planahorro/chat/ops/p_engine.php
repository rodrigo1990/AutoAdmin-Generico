...
<!--
	...
	function requesting()
	{
		...
		if ( !reconnect )
			st_network = setTimeout( function(){ stopit(0) ; parent.check_network( 715, this.undefined, this.undefined ) }, parseInt( <?php echo $VARS_JS_OP_CONSOLE_TIMEOUT ?> ) * 1000 ) ;
		else
			st_network = setTimeout( function(){ stopit(0) ; parent.check_network( 717, this.undefined, this.undefined ) }, parseInt( <?php echo $VARS_JS_REQUESTING ?> ) * 1000 ) ;

		$.ajax({
		type: "GET",
		url: parent.base_url_full+"/ajax/chat_requesting.php",
		...
		success: function(data){
			...
			try {
				...
				if ( !reconnect )
				{
					parent.check_network( 716, this.undefined, this.undefined ) ;
				} return false ;
			}

			chatting_err_815 = this.undefined ;
			if ( !stopped || ( stopped && reconnect ) )
			{
				...
				{
					write_debug( "<br>[ ERROR: p_engine requesting status 0 ] " ) ;
				}
				++c_requesting ;
		...
		error:function (xhr, ajaxOptions, thrownError){
			...
			{
				stopit(0) ;
				st_reconnect = setTimeout(function(){ parent.check_network( 815+":"+xhr.status, this.undefined, this.undefined ) ; }, 3000) ;
			}
		} });
	...
	function chatting()
	{
		...
		if ( start )
		{
			...
			$.ajax({
			type: "GET",
			url: parent.base_url_full+"/ajax/chat_chatting.php",
			data: "action=chatting&requestid="+requestid+"&t_vses="+t_vses+"&isop="+isop+"&isop_="+isop_+"&isop__="+isop__+"&ces="+thisces+"&c_chatting="+c_chatting+"&"+q_ces+q_chattings+q_cids+q_isop_+q_isop__+"&m="+mobile+"&"+unique,
			success: function(data){
				try {
					eval(data) ;
				} catch(err) {
				...
				chatting_err_915 = this.undefined ;
				if ( !stopped || ( stopped && reconnect ) )
				{
					stopped = 0 ; // reset it for disconnect situation
					reconnect = 0 ;
			...
			error:function (xhr, ajaxOptions, thrownError){
				if ( isop )
				{
					...
					{
						stopit(0) ;
						st_reconnect = setTimeout(function(){ parent.check_network( 915+":"+xhr.status, this.undefined, this.undefined ) ; }, 1000) ;
					}
				}
