		...
		if ( theoption )
		{
			...
			success: function(data){
				...
				if ( json_data.status )
				{
					if ( thevalue )
					{
						global_auto_canid = thecanid ;
						parent.auto_can_title = json_data.title ;
					}
					else
					{
						global_auto_canid = 0 ;
						parent.auto_can_title = "" ;
	...
	<tr>
		<td class="t_ml"></td><td class="t_mm">
			<div style="padding-bottom: 15px;">
				<div class="menu_traffic_info_focus" style="font-weight: normal; border: 0px; cursor: default;">&bull; canned responses created here will be available to your account only &nbsp; <span class="chat_info_td_t" onClick="toggle_new(1)" style="cursor: pointer;"><img src="../themes/<?php echo $opinfo["theme"] ?>/add.png" width="12" height="12" border="0" alt=""> Add New</span></div>
				...
				<?php
					for ( $c = 0; $c < count( $cans ); ++$c )
					{
						...
						if ( isset( $dept_hash[$can["deptID"]] ) )
						{
							...
							$auto_can_checked = ( $can["auto_select"] ) ? "checked" : "" ;
							$auto_can_global = ( $auto_can_checked ) ? "<script type=\"text/javascript\">global_auto_canid=$can[canID]</script>" : "" ;
							...
							$auto_can_div = ( $can["opID"] != 1111111111 ) ? "<div style=\"text-align: center;\" class=\"chat_info_td_traffic\">$auto_can_global<input type=\"checkbox\" id=\"auto_can_$can[canID]\" value=\"$can[canID]\" onClick=\"select_auto_can($can[canID])\" $auto_can_checked></div>" : "" ;
