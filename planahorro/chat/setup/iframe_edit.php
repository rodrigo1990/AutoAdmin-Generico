		...
		parent.do_alert( 1, "Success" ) ;
			<?php if ( $sub == "hours" ): ?>
				<?php if ( $offline_hours_onoff ): ?>
					<?php if ( $copy_all ): ?>
					$('*[id*=auto_offline_img_]', parent.document).each(function() {
						$(this).attr( 'src', '../pics/icons/online_green.png' ) ;
					}) ;
					<?php else: ?>
					$('#auto_offline_img_<?php echo $deptid ?>', parent.document).attr( 'src', '../pics/icons/online_green.png' ) ;
					<?php endif ; ?>
				<?php else: ?>
					<?php if ( $copy_all ): ?>
					$('*[id*=auto_offline_img_]', parent.document).each(function() {
						$(this).attr( 'src', '../pics/icons/bullet_red.png' ) ;
					}) ;
					<?php else: ?>
					$('#auto_offline_img_<?php echo $deptid ?>', parent.document).attr( 'src', '../pics/icons/bullet_red.png' ) ;
					<?php endif ; ?>
				<?php endif ; ?>
			<?php endif ; ?>
		<?php elseif ( $error ): ?>
...
<div id="iframe_body" style="height: 350px;">
		...
		<div style="">
			...
			<div id="info_temail" style="display: none;">
				...
				<div style="margin-top: 15px;">
					<div><img src="../pics/icons/email.png" width="16" height="16" border="0" alt=""> Send Transcript From: </div>
					<div style="margin-top: 5px;">
						<div class="li_op round"><input type="radio" name="femail" id="femail_op" value="0" <?php echo ( !isset( $deptvars["trans_f_dept"] ) || !$deptvars["trans_f_dept"] ) ? "checked" : "" ; ?>> Operator Email Address</div>
						<div class="li_op round"><input type="radio" name="femail" id="femail_dept" value="1" <?php echo ( isset( $deptvars["trans_f_dept"] ) && $deptvars["trans_f_dept"] ) ? "checked" : "" ; ?>> Department Email Address</div>
		...
		<div style="">
					...
					<tr>
						<td valign="top">
							...
							<div style="margin-top: 10px;">
								<div class="info_error" style="float: left; width: 60px; padding: 3px;"><input type="radio" name="offline_hours_onoff" id="offline_hours_onoff_off" value=0 onmousedown="toggle_offline_hours(0)"> Off</div>
								<div class="info_good" style="float: left; margin-left: 10px; width: 60px; padding: 3px;"><input type="radio" name="offline_hours_onoff" id="offline_hours_onoff_on" value=1 onmousedown="toggle_offline_hours(1)"> On</div>
								<div style="clear: both;"></div>
							</div>
						</td>
								...
								<td>
									...
									<tr>
										...
										<td style="padding-left: 25px;">
											<div class="info_neutral">the current <a href="settings.php?ses=<?php echo $ses ?>&jump=time" target="_parent">system time</a> is <span id="span_system_time" style="color: #79C2EB; font-size: 16px; font-weight: bold;"><?php echo date( "g:i a", time() ) ; ?></span></div>
										</td>
									</tr>
									</table>
								</td>
			...
			<div style="padding-bottom: 15px; text-align: justify;">
				<div style="float: left; min-height: 180px; width: 370px;" class="info_info">
					...
					<div style="margin-top: 10px;">
						<div class="li_op round"><input type="radio" name="rquestion" id="rquestion_1" value="1" checked> Yes</div>
						<div class="li_op round"><input type="radio" name="rquestion" id="rquestion_0" value="0"> No</div>
						<div style="clear: both;"></div>
					</div>
				</div>
				<div style="float: left; margin-left: 2px; min-height: 180px; width: 370px;" class="info_info">
					<div>Add a custom field on the chat request window.</div>
					<div style="margin-top: 10px; font-size: 10px; text-shadow: none;" class="info_box">Current fields are Name, Email and Question.  Additional field to display to the visitor could be "Login", "Phone", "Order Number", etc.</div>
					<div style="margin-top: 10px;">
						<table cellspacing=0 cellpadding=0 border=0>
						<tr>
			...
			<!--
				...
				$( "input#rquestion_"+<?php echo $deptinfo["rquestion"] ?> ).attr( "checked", true ) ;
			//-->
			</script>
							...
							<div style="margin-top: 5px;">
								...
								<tr>
									<td valign="top" width="150">
										<div>
											<div class="info_error" style="float: left; width: 60px; padding: 3px;"><input type="radio" name="idle_disconnect_o_onoff" id="idle_disconnect_o_onoff_off" value=0 onmousedown="toggle_idle_minutes_o(0)"> Off</div>
											<div class="info_good" style="float: left; margin-left: 10px; width: 60px; padding: 3px;"><input type="radio" name="idle_disconnect_o_onoff" id="idle_disconnect_o_onoff_on" value=1 onmousedown="toggle_idle_minutes_o(1)"> On</div>
											<div style="clear: both;"></div>
										</div>
									</td>
									<td style="padding-left: 15px;">
						...
						<div style="" class="info_neutral">
							...
							<div style="margin-top: 5px;">
								...
								<tr>
									<td valign="top" width="150">
										<div>
											<div class="info_error" style="float: left; width: 60px; padding: 3px;"><input type="radio" name="idle_disconnect_v_onoff" id="idle_disconnect_v_onoff_off" value=0 onmousedown="toggle_idle_minutes_v(0)"> Off</div>
											<div class="info_good" style="float: left; margin-left: 10px; width: 60px; padding: 3px;"><input type="radio" name="idle_disconnect_v_onoff" id="idle_disconnect_v_onoff_on" value=1 onmousedown="toggle_idle_minutes_v(1)"> On</div>
											<div style="clear: both;"></div>
										</div>
									</td>
									<td style="padding-left: 15px;">
	...
	<div id="canned_list" style="overflow: auto;">
		...
		<?php
			for ( $c = 0; $c < count( $cans )-1; ++$c )
			{
				...
				print "<tr id=\"tr_div_$can[canID]\"><td class=\"$td1\" nowrap><div onClick=\"do_edit($can[canID], '$title', '$can[deptID]', '$message')\" style=\"cursor: pointer;\"><img src=\"../pics/btn_edit.png\" width=\"45\" height=\"16\" border=\"0\" alt=\"\"></div><div onClick=\"do_delete($can[canID])\" style=\"margin-top: 5px; cursor: pointer;\"><img src=\"../pics/btn_delete.png\" width=\"45\" height=\"16\" border=\"0\" alt=\"\"></div></td><td class=\"$td1\"><b>$title_display</b></td><td class=\"$td1\" nowrap>$op_name</td><td class=\"$td1\">$dept_name</td><td class=\"$td1\">$message_display</td></tr>" ;
			}
			if ( $c == 0 )
	...
	<div id="canned_box_new" style="display: none; position: fixed; top: 0px; left: 0px; background: #FFFFFF; height: 100%; padding: 5px; z-Index: 5;">
		...
		<table cellspacing=0 cellpadding=0 border=0 width="100%">
		<tr>
			<td valign="top" width="380" style="padding: 5px;" nowrap>
				<input type="hidden" name="ses" value="<?php echo $ses ?>">
				<input type="hidden" name="action" value="update">
				<input type="hidden" name="sub" value="<?php echo $action ?>">
