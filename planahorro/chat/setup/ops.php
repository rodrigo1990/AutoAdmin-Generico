<?php
	...
	if ( $action == "submit" )
	{
		...
		$maxc = Util_Format_Sanatize( Util_Format_GetVar( "maxc" ), "ln" ) ;
		...
		{
			$error = Ops_put_Op( $dbh, $opid, 1, $rate, $sms, $op2op, $traffic, $viewip, $maxc, $login, $password, $name, $email ) ;
...
<!--
	...
	function do_edit( theopid, thename, theemail, thelogin, therate, thesms, theop2op, thetraffic, theviewip, themaxc )
	{
...
<?php include_once( "./inc_header.php" ) ?>
			...
			<div id="div_list" style="margin-top: 25px;">
				...
				<?php
					...
					for ( $c = 0; $c < count( $operators ); ++$c )
					{
						...
						$status_img = ( $operator["status"] ) ? "<img src=\"../pics/icons/bulb.png\" width=\"16\" height=\"16\" border=\"0\" alt=\"online\" title=\"online\"><br>close" : "<img src=\"../pics/icons/bulb_off.png\" width=\"16\" height=\"16\" border=\"0\" alt=\"offline\" title=\"offline\">" ;
						...
						<tr id=\"div_tr_$operator[opID]\">
							...
							<td class=\"$td1\">
								...
								<div class=\"info_neutral\" style=\"margin-top: 5px; font-size: 10px; width: 45px;\">ID: $operator[opID]</div>
							</td>
		...
		<div style="display: none;" id="ops_online">
			<div style="margin-top: 25px;">
				...
				<div class="info_info" style="margin-top: 15px;">
					...
					<div style="margin-top: 5px; font-size: 32px; font-weight: bold; text-shadow: 1px 1px #FFFFFF;"><a href="<?php echo ( !preg_match( "/^(http)/", $CONF["BASE_URL"] ) ) ? "http$https:$CONF[BASE_URL]" : $CONF["BASE_URL"] ; ?>" target="_blank" style="color: #8DB173; text-shadow: 1px 1px #FFFFFF;" class="nounder"><?php echo ( !preg_match( "/^(http)/", $CONF["BASE_URL"] ) ) ? "http$https:$CONF[BASE_URL]" : $CONF["BASE_URL"] ; ?></a></div>
				</div>

				<div style="margin-top: 25px;"><img src="../pics/icons/info.png" width="12" height="12" border="0" alt=""> Don't forget to place the chat icon <a href="./code.php?ses=<?php echo $ses ?>">HTML Code</a> onto your webpages.</div>
				<?php endif ; ?>
			</div>
		</div>
		...
		<div id="div_form" style="display: none; margin-top: 25px;" id="a_edit">
			...
			<div style="margin-top: 15px;">
				...
				<tr>
					...
					<td style="padding-left: 10px;"><input type="text" name="login" id="login" size="30" maxlength="160" value="" onKeyPress="return nospecials(event)"></td>
				</tr>
				</table>
			</div>
			<div style="margin-top: 15px;">
			...
			<div style="margin-top: 15px;">
				<table cellspacing=0 cellpadding=0 border=0>
				<tr>
					<td><div class="tab_form_title">Concurrent Active Chat Sessions<div style="font-size: 10px; font-weight: normal;">Does not apply to department <a href="./depts.php?ses=<?php echo $ses ?>">simultaneous routing type</a>.</div></div></td>
					<td style="padding-left: 10px;">For new chat requests, automatically skip this operator when the operator has reached
						...
						</select> concurrent <span class="info_box">active chat sessions</span>.
					</td>
				</tr>
				</table>
			</div>
		...
		<div id="div_notice_disconnect" style="display: none; position: absolute; text-align: right;" class="info_error">
			<div style="padding: 10px;">
				<div class="edit_title"><span class="info_good">Operator <span id="span_login_disconnect"></span> is ONLINE.</span>  Remote disconnect operator console and go offline?</div>
		...
		<div id="div_notice_delete" style="display: none; position: absolute;" class="info_error">
			<div style="padding: 10px;">
				...
				<div style="margin-top: 5px;">Deleting the operator account will also delete the operator's transcripts.</div>

				<div style="margin-top: 5px;"><button type="button" onClick="do_delete_doit()">Delete</button> &nbsp; <a href="JavaScript:void(0)" style="color: #FFFFFF" onClick="$('#div_notice_delete').fadeOut('fast')">cancel</a></div>
			</div>
		</div>
