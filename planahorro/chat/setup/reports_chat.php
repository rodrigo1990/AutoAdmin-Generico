...
<!--
	...
	var global_div = "depts" ;
	var global_timeline_unix = 0 ;
	var global_deptid = 0 ;
	var global_opid = 0 ;
	var global_clicktype ;
	...
	function populate_urls( theunix )
	{
		...
		$.ajax({
			...
			success: function(data){
				...
				var url ;
				for ( c = 0; c < json_data.urls.length; ++c )
				{
	...
	function populate_timeline( theunix )
	{
		$('#reports_timeline_body').empty().html( "<img src=\"../pics/loading_fb.gif\" width=\"16\" height=\"11\" border=\"0\" alt=\"\">" ) ;
		$.ajax({
			type: "POST",
			url: "../ajax/setup_actions_reports.php",
			data: "ses=<?php echo $ses ?>&action=fetch_request_timeline&deptid="+global_deptid+"&opid="+global_opid+"&sdate="+theunix+"&stat_start=<?php echo $stat_start ?>&stat_end=<?php echo $stat_end ?>&"+unixtime(),
			success: function(data){
				eval( data ) ;
				...
				update_timeline_string( theunix, hour_total_overall ) ;

				for ( c = 0; c < json_data.timeline.length; ++c )
				{
					...
					else
					{
						...
						string_js = "onClick=\"global_clicktype=2;update_timeline_string('"+hour["hour_display"]+"', "+hour["total"]+")\"" ;
					}

					string_hours += "<td valign=\"bottom\" width=\"2%\"><div id=\"bar_v_requests_"+c+"\" title=\""+tooltip_display+"\" alt=\""+tooltip_display+"\" style=\"height: "+h1+"; background: url( ../pics/meters/"+meter+" ) repeat; border-top-left-radius: 5px 5px; -moz-border-radius-topleft: 5px 5px; border-top-right-radius: 5px 5px; -moz-border-radius-topright: 5px 5px;"+string_cursor+"\" "+string_js+"></div></td><td><img src=\"../pics/space.gif\" width=\"5\" height=1></td>" ;
				}
				string_hours += "</tr><tr>" ;
				...
				for ( c = 0; c < json_data.timeline.length; ++c )
				{
					...
					string_hours += "<td align=\"center\"><div id=\"requests_bg_day\" class=\"page_report\" style=\"margin: 0px; font-size: 10px; font-weight: bold;\" title=\""+tooltip_display+"\" id=\""+tooltip_display+"\" onClick=\"global_clicktype=2;update_timeline_string('"+hour["hour_display"]+"', "+hour["total"]+")\">"+hour["hour_"]+hour["ampm"]+"</div></td><td><img src=\"../pics/space.gif\" width=\"5\" height=1></td>" ;
				}
				string_hours += "</tr><tr>" ;
	...
	function switch_dept( thedeptid )
	{
		...
		populate_timeline( global_stat_time ) ;
	}
	...
	function update_timeline_string( theunixtime, thetotal )
	{
		var day_string = $('#stat_day_expand').html() ;

		if ( ( global_clicktype == 1 ) || ( typeof( theunixtime ) != "string" ) )
			$('#stat_timeline_expand').html( "<span class=\"info_box\">"+day_string+" <span style=\"font-weight: bold;\">(12:00am - 11:59pm)</span></span> <span class=\"info_good\">Requests: "+thetotal+"</span>" ) ;
		else
		{
			var day_string_output = theunixtime.replace( "%stat_day%", day_string ) ;
			day_string_output = day_string_output.replace( "%span%", "<span style=\"font-weight: bold;\">" ) ;
			day_string_output = day_string_output.replace( "%span_%", "</span>" ) ;
			$('#stat_timeline_expand').html( "<span class=\"info_box\">"+day_string_output+"</span> <span class=\"info_good\"><img src=\"../pics/icons/chats.png\" width=\"16\" height=\"16\" border=\"0\" alt=\"\" style=\"position: relative; top: 5px;\"> Requests: "+thetotal+"</span>" ) ;
		}
	}
//-->
</script>
...
<?php include_once( "./inc_header.php" ) ?>
			...
			<tr>
				<?php
					for ( $c = 1; $c <= $stat_end_day; ++$c )
					{
						...
						$stat_day_expand = date( "l, M j, Y", $stat_day ) ;
						print "
							<td align=\"center\"><div id=\"requests_bg_day\" class=\"page_report\" style=\"min-width: 12px; margin: 0px; font-size: 10px; font-weight: bold;\" title=\"$tooltips[$stat_day]\" id=\"$tooltips[$stat_day]\" OnClick=\"global_clicktype=1;select_date( $stat_day, '$stat_day_expand' );\">$c</div></td>
							<td><img src=\"../pics/space.gif\" width=\"5\" height=1 border=0></td>
						" ;
					}
				?>
		...
		<div id="overview_day_chart" style="margin-top: 50px;">
			<div id="overview_date_title"><div id="stat_day_expand" class="info_box"></div></div>

			<div class="op_submenu_wrapper">
				<div class="op_submenu_focus" onClick="show_div('depts')" id="sub_menu_depts">Departments</div>
				<div class="op_submenu2" onClick="show_div('ops')" id="sub_menu_ops">Operators</div>
				...
				<div class="op_submenu2" onClick="show_div('info')" id="sub_menu_info"><span class="info_box"><img src="../pics/icons/info.png" width="12" height="12" border="0" alt=""> Information About the Chat Stats</span></div>
				<div style="clear: both"></div>
			</div>

			<div id="overview_data_container" style="margin-top: 15px;">
				<div id="reports_depts" style="min-height: 300px; max-height: 350px; overflow: auto; overflow-x: hidden;">
					...
					<?php
						for ( $c = 0; $c < count( $departments ); ++$c )
						{
							...
							print "
								<tr>
									<td class=\"$td1\" nowrap><a href=\"JavaScript:void(0)\" onClick=\"department_timeline($department[deptID])\">$department[name]</a></td>
				...
				<div id="reports_ops" style="display: none; min-height: 300px; max-height: 350px; overflow: auto;">
					<table cellspacing=0 cellpadding=0 border=0 width="100%">
					<tr>
				...
				<div id="reports_info" style="display: none; min-height: 300px; max-height: 350px; overflow: auto;">
					<div class="info_box">
						<div style="font-size: 14px; font-weight: bold;"><img src="../pics/icons/info.png" width="14" height="14" border="0" alt=""> For one chat request, the following report will be stored:</div>
						<ul style="margin-top: 15px;">
							<li> For every operator that receives the chat request, their <b>"Requests"</b> value will increment by one.
					...
					<div style="margin-top: 15px;" class="info_info">
						<div><img src="../pics/icons/reset.png" width="16" height="16" border="0" alt=""> <b>Chat Stats Reset</b></div>
						<div style="margin-top: 5px;">Chat stats can be reset, clearing the data and setting the values to zero across all departments and operators.  The <a href="JavaScript:void(0)" onClick="show_div('timeline')">Chat Request Timeline</a>, <a href="JavaScript:void(0)" onClick="show_div('urls')">Chat Request URLs</a> and the <a href="reports_chat_missed.php?ses=<?php echo $ses ?>">Missed Chats</a> data will also be cleared.  Chat stats reset does not affect the transcripts, offline messages, or any other portion of the system.</div>
