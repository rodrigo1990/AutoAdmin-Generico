<?php
	...
	function Util_Format_Sanatize( $string, $flag )
	{
		switch ( $flag )
		{
			...
			case ( "n" ):
				return preg_replace( "/[^0-9.]/i", "", trim( $string, "\x00" ) ) ; break ;
			case ( "ln" ):
				return preg_replace( "/[`\$*%=<>\(\)\[\]\|\{\}\/\\\]/i", "", trim( $string, "\x00" ) ) ; break ;
			case ( "lns" ):
				return preg_replace( "/[^a-z0-9]/i", "", trim( $string, "\x00" ) ) ; break ;
			case ( "e" ):
				return preg_replace( "/[^a-z0-9_.\-@]/i", "", Util_Format_Trim( trim( $string, "\x00" ) ) ) ; break ;
			case ( "v" ):
				return preg_replace( "/(%20)|(%00)|(%3Cv%3E)|(<v>)/", "", Util_Format_Trim( trim( $string, "\x00" ) ) ) ; break ;
			case ( "base_url" ):
				return preg_replace( "/[\$\!`\"<>'\?;]/i", "", Util_Format_Trim( preg_replace( "/hphp/i", "http", trim( $string, "\x00" ) ) ) ) ; break ;
			case ( "url" ):
				return preg_replace( "/[\$\!`\"<>'\(\); ]/i", "", Util_Format_Trim( preg_replace( "/hphp/i", "http", trim( $string, "\x00" ) ) ) ) ; break ;
			case ( "title" ):
				return preg_replace( "/[`\$=\!<>;]/i", "", Util_Format_Trim( preg_replace( "/hphp/i", "http", trim( $string, "\x00" ) ) ) ) ; break ;
			case ( "htmltags" ):
				return Util_Format_ConvertTags( trim( $string, "\x00" ) ) ; break ;
			case ( "timezone" ):
				return preg_replace( "/['`?\$*%=<>\(\)\[\]\|\{\}\\\]/i", "", trim( $string, "\x00" ) ) ; break ;
			case ( "notags" ):
				return strip_tags( trim( $string, "\x00" ) ) ; break ;
			default:
	...
	{
		global $CONF ;
		file_put_contents( "$CONF[CONF_ROOT]/debug.txt", $string, FILE_APPEND ) ;
	}
