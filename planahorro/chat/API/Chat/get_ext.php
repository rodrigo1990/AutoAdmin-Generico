<?php
						...
						$vis_token )
	{
		...
		$query = "SELECT * FROM p_transcripts WHERE md5_vis = '$vis_token' ORDER BY created DESC" ;
		database_mysql_query( $dbh, $query ) ;
								...
								$limit )
	{
		...
		$search_string = "" ;
		if ( strlen( $text ) == 32 )
			$search_string = " OR ces = '$text' OR md5_vis = '$text' " ;
		if ( $text ) { $search_string = " AND ( plain LIKE '%$text%' $search_string ) " ; }
								...
								$limit )
	{
		...
		if ( strlen( $text ) == 32 )
			$search_string = " OR ces = '$text' OR md5_vis = '$text' " ;
		if ( $text ) { $search_string = " ( plain LIKE '%$text%' $search_string ) AND " ; }
								...
								$stat_end )
	{
		...
		$query = "SELECT * FROM p_req_log WHERE created >= $stat_start AND created <= $stat_end $dept_string $op_string AND op2op = 0 AND initiated = 0 AND archive = 0" ;
		database_mysql_query( $dbh, $query ) ;
