<?php
	...
	FUNCTION Chat_remove_itr_OldRequests( &$dbh )
	{
		...
		if ( $total_files )
		{
			...
			{
				preg_match( "/(.*?)\.txt/", $dir_files[$c], $matches ) ;
				if ( isset( $matches[1] ) )
				{
		...
		if ( $dbh[ 'ok' ] )
		{
			if ( database_mysql_nresults( $dbh ) || $total_files )
			{
				...
				if ( count( $expired_requests ) )
				{
					...
					for ( $c = 0; $c < count( $expired_requests ); ++$c )
					{
						...
						$ces = $request["ces"] ; $ip = $request["ip"] ; $vis_token = $request["md5_vis"] ; $deptid = $request["deptID"] ;
						$trans_file = "$ces.txt" ;
