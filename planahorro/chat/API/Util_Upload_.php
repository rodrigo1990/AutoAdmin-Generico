<?php
	...
	function Util_Upload_GetLogo( $deptid )
	{
		...
		$now = time() ; $custom = 0 ; $image_path = "$CONF[DOCUMENT_ROOT]/pics/logo.png" ; $image_type = "image/png" ;
		if ( is_file( "$CONF[CONF_ROOT]/logo"."_$deptid.GIF" ) ) { $custom = 1 ;
			$image_path = "$CONF[CONF_ROOT]/logo"."_$deptid.GIF" ; $image_type = "image/gif" ; }
		else if ( is_file( "$CONF[CONF_ROOT]/logo"."_$deptid.JPEG" ) ) { $custom = 1 ;
			$image_path = "$CONF[CONF_ROOT]/logo"."_$deptid.JPEG" ; $image_type = "image/jpeg" ; }
		else if ( is_file( "$CONF[CONF_ROOT]/logo"."_$deptid.PNG" ) ) { $custom = 1 ;
			$image_path = "$CONF[CONF_ROOT]/logo"."_$deptid.PNG" ; $image_type = "image/png" ; }
		else if ( is_file( "$CONF[CONF_ROOT]/logo_0.GIF" ) ) { $custom = 1 ;
			$image_path = "$CONF[CONF_ROOT]/logo_0.GIF" ; $image_type = "image/gif" ; }
		else if ( is_file( "$CONF[CONF_ROOT]/logo_0.JPEG" ) ) { $custom = 1 ;
			$image_path = "$CONF[CONF_ROOT]/logo_0.JPEG" ; $image_type = "image/jpeg" ; }
		else if ( is_file( "$CONF[CONF_ROOT]/logo_0.PNG" ) ) { $custom = 1 ;
			$image_path = "$CONF[CONF_ROOT]/logo_0.PNG" ; $image_type = "image/png" ; }
		else if ( is_file( "$CONF[DOCUMENT_ROOT]/themes/$local_theme/logo.png" ) ) { $custom = 1 ;
			$image_path = "$CONF[DOCUMENT_ROOT]/themes/$local_theme/logo.png" ; $image_type = "image/png" ; }
		return Util_Upload_Output( $custom, $deptid, $image_path, $image_type ) ;
	}
