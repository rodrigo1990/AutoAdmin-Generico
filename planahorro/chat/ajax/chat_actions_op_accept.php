		...
		else
		{
			...
			if ( $requestinfo["status"] != 2 )
			{
				...
				else
				{
					...
					if ( !$requestinfo["initiated"] ) { Ops_put_itr_OpReqStat( $dbh, $requestinfo["deptID"], $opid_cookie, "taken", 1 ) ; }
					...
					else if ( $requestinfo["opID"] && ( $requestinfo["opID"] != 1111111111 ) )
					{
						...
						UtilChat_AppendToChatfile( "$filename.text", "<idle_restart>--tback--".$text."<idle_restart>" ) ;
					}
				}
			}
			else
