<?php
	...
	else if ( $action == "fetch_request_urls" )
	{
		...
		{
			$stat_start = Util_Format_Sanatize( Util_Format_GetVar( "stat_start" ), "ln" ) ;
			$stat_end = Util_Format_Sanatize( Util_Format_GetVar( "stat_end" ), "ln" ) ;
		}
	...
	else if ( $action == "fetch_request_timeline" )
	{
		...
		for ( $c = 0; $c < count( $timeline ); ++$c )
		{
			...
			if ( isset( $hours[$hour] ) ) { ++$hours[$hour] ; }
			else { $hours[$hour] = 1 ; }
		}

		$now = time() ; $total_overall = $max = 0 ;
		...
		for ( $c = 0; $c <= 23; ++$c )
		{
			...
			$ampm = date( "a", $now_ ) ;
			$hour_ = date( "g", $now_ ) ;
			$hour_display = "%stat_day% %span%($hour_:00$ampm - $hour_:59$ampm)%span_%" ;

			$unixtime = $now_ ;
			if ( isset( $hours[$c] ) )
			{
				$total_overall += $hours[$c] ;
				if ( $hours[$c] > $max ) { $max = $hours[$c] ; }
				$json_data .= "{ \"hour\": \"$c\", \"timestamp\": \"$unixtime\", \"hour_display\": \"$hour_display\", \"ampm\": \"$ampm\", \"hour_\": \"$hour_\", \"total\": \"$hours[$c]\" }," ;
			}
			else { $json_data .= "{ \"hour\": \"$c\", \"timestamp\": \"$unixtime\", \"hour_display\": \"$hour_display\", \"ampm\": \"$ampm\", \"hour_\": \"$hour_\", \"total\": \"0\" }," ; }
		}

		$json_data = substr_replace( $json_data, "", -1 ) ;
		$json_data .= "	], \"hour_max\": \"$max\", \"total_overall\": \"$total_overall\" };" ;
	}
	else
