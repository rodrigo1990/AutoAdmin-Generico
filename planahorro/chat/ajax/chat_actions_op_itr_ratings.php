	...
	else if ( $action == "fetch_ratings" )
	{
		...
		if ( isset( $opinfo["ses"] ) && ( $opinfo["ses"] == $ses ) )
		{
			...
			if ( $status )
			{
				...
				{
					include_once( "$CONF[DOCUMENT_ROOT]/API/Ops/update.php" ) ;
					Ops_update_PutOpStatus( $dbh, $opid, 1 ) ;
				}
			}

			$json_data = "json_data = { \"status\": 1, \"rating_overall\": \"$overall\", \"rating_recent\": \"$opinfo[rating]\", \"ces\": \"$opinfo[ces]\", \"chats_today\": $chats_today, \"chats_overall\": $chats_overall, \"op_status\": $opinfo[status], \"signal\": $signal }; " ;
		}
		else if ( $opinfo["ses"] != $ses )
			$json_data = "json_data = { \"status\": 0, \"rating_overall\": \"\", \"rating_recent\": \"\", \"ces\": \"\", \"signal\": 3 }; " ;
		else
			$json_data = "json_data = { \"status\": 0, \"rating_overall\": \"\", \"rating_recent\": \"\", \"ces\": \"\", \"signal\": 1 }; " ;
	}
	else
