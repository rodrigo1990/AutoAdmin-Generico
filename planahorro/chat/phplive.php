<?php
	...
	if ( !$token ) { HEADER( "location: ./fetch_token.php?$query" ) ; exit ; }
...
<!--
	...
	$(document).ready(function( )
	{
		...
		$('#chat_button_start').html( "<?php echo utf8_encode( $LANG["CHAT_BTN_START_CHAT"] ) ?>" ).unbind('click').bind('click', function( ) {
			start_chat( ) ;
		}) ;
	...
	function init_divs_pre( )
	{
		...
		var deptid_width = body_width - 15 ;
		var powered_bottom = $('#chat_footer').height( ) + 25 ;
		var input_width = Math.floor( body_width/2 ) - 30 ;
	...
	function select_dept( thevalue )
	{
		...
		if ( thevalue && ( typeof( dept_customs[thevalue] ) != "undefined" ) )
		{
			...
			var optional_string = ( custom_required ) ? "" : " (<?php echo utf8_encode( $LANG["TXT_OPTIONAL"] ) ?>)" ;
		...
		if ( ( ( typeof( dept_settings[thevalue] ) != "undefined" ) && dept_settings[thevalue][0] ) || !thevalue )
			...
			$('#optional_email').html( " (<?php echo utf8_encode( $LANG["TXT_OPTIONAL"] ) ?>)" ) ;
		...
		if ( ( $('#vdeptid option:selected').attr( "class" ) == "offline" ) )
		{
			...
			$('#chat_button_start').html( "<?php echo utf8_encode( $LANG["CHAT_BTN_EMAIL"] ) ?>" ).unbind('click').bind('click', function( ) {
				send_email( ) ;
			});
		}
		else
		{
			onoff = 1 ;
			$('#chat_text_header_sub').html( "<?php echo preg_replace( "/\"/", "&quot;", utf8_encode( $LANG["CHAT_WELCOME_SUBTEXT"] ) ) ?>" ) ;
			...
			{
				$('#chat_button_start').html( "<?php echo utf8_encode( $LANG["TXT_SUBMIT"] ) ?>" ).attr( "disabled", false ).unbind('click').bind('click', function( ) {
					start_chat( ) ;
				});
			}
		}
	...
	function check_form( theflag )
	{
		...
		if ( !deptid ){
			do_alert( 0, "<?php echo utf8_encode( $LANG["CHAT_JS_BLANK_DEPT"] ) ?>" ) ;
			return false ;
		}
		if ( !$('#vsubject').val( ) ){
			if ( theflag )
			{
				do_alert( 0, "<?php echo utf8_encode( $LANG["CHAT_JS_BLANK_SUBJECT"] ) ?>" ) ;
				return false ;
			}
		}
		...
		if ( !$('#vname').val( ) ){
			do_alert( 0, "<?php echo utf8_encode( $LANG["CHAT_JS_BLANK_NAME"] ) ?>" ) ;
			return false ;
		}
		if ( !$('#vemail').val( ) ){
		...
		if ( !$('#vquestion').val( ) ){
			...
			{
				do_alert( 0, "<?php echo utf8_encode( $LANG["CHAT_JS_BLANK_QUESTION"] ) ?>" ) ;
				return false ;
			}
		}
		...
		if ( custom_required && !$('#custom_field_input_1').val( ) ){
			do_alert( 0, "<?php echo utf8_encode( $LANG["CHAT_JS_CUSTOM_BLANK"] ) ?>"+" "+$('#custom_field_title').html( ) ) ;
			return false ;
		}
	...
	function start_chat( )
	{
		if ( check_form(0) )
		{
			...
			if ( <?php echo ( !isset( $requestinfo["requestID"] ) ) ? 1 : 0 ; ?> && <?php echo $embed ?> && ( typeof( parent.chat_connected ) != "undefined" ) )
			{
				...
				setTimeout( function(){ $('#chat_button_start').attr( "disabled", true ).html( "<?php echo utf8_encode( $LANG["TXT_CONNECTING"] ) ?>" ) ; }, 1000 ) ;
				// delay of few seconds before re-enable button to try again
				setTimeout( function(){ $('#chat_button_start').attr( "disabled", false ).html( "<?php echo utf8_encode( $LANG["CHAT_BTN_START_CHAT"] ) ?>" ) ; }, 6000 ) ;
			}
		...
		if( check_form(1) )
		{
			...
			success: function(jdata){
				...
				if ( json_data.status )
				{
					...
					$('#chat_button_start').attr( "disabled", true ) ;
					$('#chat_button_start').html( "<img src=\"./themes/<?php echo $theme ?>/alert_good.png\" width=\"16\" height=\"16\" border=\"0\" alt=\"\"> <?php echo utf8_encode( $LANG["CHAT_JS_EMAIL_SENT"] ) ?>" ) ;
				}
				else
				{
					do_alert( 0, json_data.error ) ;
					$('#chat_button_start').html( "<?php echo utf8_encode( $LANG["CHAT_BTN_EMAIL"] ) ?>" ) ;
				}
			},
...
<div style="position: absolute; top: 2px; padding: 10px; z-Index: 2;">
	<div id="chat_body" style="padding: 10px; overflow-y: auto; overflow-x: hidden;">

		<?php if ( !$embed ): ?>
		<div id="chat_logo" style="padding-bottom: 15px;"><img src="<?php echo Util_Upload_GetLogo( $deptid ) ?>" border=0></div>
		<?php endif ; ?>
